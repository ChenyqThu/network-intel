...
- `context.prior_coverage`：该信号过去是否已报道。若高度重合，请表述为「持续 / 升级 / 拐点」，
  不要当全新信息重复。
- `context.recent_coverage`：最近 N 期**已发布报告**覆盖过的主题与来源（每条含 insight 标题 /
  💡takeaway、被引条目标题与 url）。这是**时间去重的关键依据**：本期信号若与之高度重合，必须表述为
  「**持续 / 升级 / 拐点**」并点明本期新增量，**不要当作全新事件重新展开**；除非热度或事实发生重大
  变化才值得重新详述。注意——**同一主题换个来源 url 也算重复**。
