...
- `context.prior_coverage`：该信号过去是否已报道。若高度重合，请表述为「持续 / 升级 / 拐点」，
  不要当全新信息重复。
- `context.recent_coverage`：最近 N 期**已发布周报**覆盖过的主题与来源（每条含 insight 标题 /
  💡takeaway、被引条目标题与 url）。这是**时间去重的关键依据**。宏观/行业/供应链话题变化慢，极易被
  连续多周检索到——本周信号若与近期周报高度重合，必须表述为「**持续 / 升级 / 拐点**」并只写**本周
  新增量**（新数据点、新进展、新转折），**严禁把上周已展开的主题当全新洞察重写**；除非事实或热度有
  重大变化才值得重新详述。注意——**同一主题换个来源 url 仍算重复**。
