/* ============================================================
   Dashboard charts (static, hi-fi SVG). Ported from
   project/charts.jsx. Style variants: 'minimal' | 'filled'.
   ============================================================ */
import { Icon } from './Icon';
import { fmtNum } from '../lib/intel';
import type { SentimentTrendPoint } from '../types';

export type ChartStyle = 'minimal' | 'filled';
...
/** Default source-slice palette (derived in the report builder). */
export const SOURCE_COLORS = [
  'var(--color-primary)',
  'color-mix(in oklab, var(--color-primary) 55%, var(--bg-app))',
  'var(--fg-tertiary)',
  'var(--border-strong)',
];
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
      <div style={{ position: 'relative', width: 150, height: 150, flex: 'none' }}>
          ...
          {data.map((d, i) => {
            ...
            const el = (
              <circle
                ...
                strokeLinecap={style === 'filled' ? 'butt' : 'round'}
              />
            );
            off += len;
            return el;
        ...
        >
          <div>
            <div className="tnum" style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-.03em', lineHeight: 1 }}>
              {sum}
            </div>
            <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--fg-tertiary)', marginTop: 3 }}>
              条信号
            </div>
          </div>
        </div>
...
/* ---- SourceBars: horizontal alternative ---- */
export function SourceBars({ data }: { data: DonutDatum[] }) {
    ...
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }} preserveAspectRatio="none">
        ...
        {grid.map((g, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={W - padX} y={y - 4} fontSize="9.5" fill="var(--fg-faint)" textAnchor="end" fontFamily="var(--font-mono)">
                {g}
              </text>
            </g>
          );
        ...
        <path
          ...
          strokeWidth="2"
          strokeDasharray={style === 'filled' ? '5 4' : '0'}
          strokeLinecap="round"
        ...
        <path
          ...
          strokeWidth={style === 'filled' ? 2.6 : 2.2}
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        {series.map((d, i) => (
      ...
      <div style={{ display: 'flex', gap: 18, fontSize: 12, marginTop: 6, color: 'var(--fg-tertiary)' }}>
        ...
        <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 14, height: 3, borderRadius: 2, background: 'var(--fg-tertiary)' }} />
          UniFi
        </span>
      </div>
    </div>
  ...
  return (
    <div className="kpi">
      ...
      <div className="val tnum">
        {value}
        {unit && (
          <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--fg-tertiary)', marginLeft: 3 }}>{unit}</span>
        )}
      </div>
      {deltaText && (
